...
All things said, TrueNAS SCALE is a great way to get started with containers and virtualization, but it has its [limitations](#limitations).

> **Hands-on note:** Everything below (Features, Installation, Application Deployment, Limitations, Known Issues) was written and exercised against **TrueNAS SCALE 22.12.1** (Kubernetes Apps / TrueCharts). It was **not** retested on later trains. From **25.04** onward, iX names the free product **TrueNAS Community Edition**; from **24.10** onward, Apps use a **Docker** backend instead of the old Kubernetes / charts model. Treat the 22.12.1 Known Issues as historical. For current ISO / docs, start from [Software Status](https://www.truenas.com/docs/softwarestatus/) and [25.10 docs](https://www.truenas.com/docs/scale/25.10/) — see also [Current trains (not retested)](#current-trains-not-retested).
...
## Current trains (not retested)

Compiled from official TrueNAS docs as of September 2026 — **not** hands-on on this machine:

*   Free product is branded **TrueNAS Community Edition** (same lineage as SCALE). Recommended stable train around **25.10.x (Goldeye)**; check [Software Status](https://www.truenas.com/docs/softwarestatus/).
*   Apps backend since **24.10** is **Docker** (catalog apps + custom Compose YAML), not the Kubernetes / TrueCharts catalog flow described above.
*   Download hub: [TrueNAS downloads](https://www.truenas.com/download/) / [Community Edition](https://www.truenas.com/download-truenas-community-edition).
*   Apps docs: [Apps Market](https://apps.truenas.com/), [Installing Custom Apps](https://apps.truenas.com/managing-apps/installing-custom-apps/).
